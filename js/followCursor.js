...
var followCursor = function followCursor(elemList) {
  ...
  // Init
  var boxSizes = [];
  var $elems = Array.from(elemList);
  var counter = 0;
  var scrollPos = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop || 0; // Get the absolute distance of an element
  ...
  var transformCSS = function transformCSS(perspective, rotateX, rotateY) {
    return "perspective(".concat(perspective, "px) rotateY(").concat(rotateY, "deg) rotateX(").concat(rotateX, "deg)");
  ...
  $elems.forEach(function (img) {
    ...
    img.addEventListener('mousemove', function (e) {
      // get key
      var count = e.currentTarget.getAttribute('data-mouserotate'); // Cursor's X position in relation to the element (0-1)
  ...
  var resetSizes = function resetSizes() {
    boxSizes.forEach(function (box, i) {
